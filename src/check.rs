...
pub fn compare(inet: &mut INet, a: &mut Cursor, b: &mut Cursor) -> bool {
  ...
  fn advance(inet: &mut INet, a: &mut Cursor, b: &mut Cursor, flip: bool) -> Option<bool> {
    ...
    // If entering main port...
      ...
      if let Some(slot) = a.pop_back(a_kind) {
        ...
        let eq = if flip { compare(inet, b, an) } else { compare(inet, an, b) };
        a.logs.pop();
  ...
  if let Some(eq) = advance(inet, a, b, false) {
    return eq;
  }

  // If 'b' can be advanced, advance it and compare
  if let Some(eq) = advance(inet, b, a, true) {
    return eq;
  }
  ...
  //println!("== {:?} {:?} {}", a.path, b.path, a.path.get(&ANN) != b.path.get(&ANN) || a.path.get(&CON) == b.path.get(&CON));
