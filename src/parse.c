...
void parse_term_sup(Parser* parser, uint32_t loc) {
  ...
  uint8_t label = parse_uint(parser) & 0x3; // Ensure it fits in 2 bits (0-3)
  ...
  // Use the appropriate SP0-SP3 tag based on label
  TermTag sup_tag = (TermTag)(SP0 + label);
...
void parse_term_col(Parser* parser, uint32_t loc) {
  ...
  // Use the appropriate CX0-CX3 and CY0-CY3 tags based on label
  Term co0_term = ic_make_col_x(label, col_node);
  Term co1_term = ic_make_col_y(label, col_node);
